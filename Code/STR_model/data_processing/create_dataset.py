...
def get_boundary_coordinates(xml_path):
    ...
    x2 = float(whiteboard_description.find('DiagonallyOppositeCoords').attrib['x']) 
...
def save_strokes(word_strokes, target_npy_path, num):
    for word in word_strokes:
        word_np = []
        min_x = min(point[0] for stroke in word for point in stroke)  # Find the minimum x-coordinate in the word
        for stroke in word:
            i = 0
            for point in stroke:
                p = [point[0] - min_x, point[1], point[2], 0, 0]  # Translate x-coordinate
                if i == 0:
                    p[3] = 1
                elif i == len(stroke) - 1:
                    p[4] = 1
...
def split_words(image_path, xml_path, target_image_path, target_npy_path, required_height, num):
    num1 = num
    num2 = num
    ...
    img_cropped = resize_image(img_cropped, required_height)
...
def main():
    num = 1
    required_height = 60 # height of the image
        ...
        for file in files:
            if file.endswith(('.tif')):
                ...
                if os.path.exists(xml_file):
                    ...
                    xml_path = xml_file
                    target_image_path = f'../../../DataSet/IAM-Online/Resized_Dataset/Train/Images/' # directory to save images
                    target_npy_path = f'../../../DataSet/IAM-Online/Resized_Dataset/Train/Strokes/' # directory to save strokes
                    try:
                        num, _ = split_words(image_path, xml_path, target_image_path, target_npy_path, required_height, num)
                ...
                # if xml file does not exist, then save the image to test folder
                    ...
                    image_path = os.path.join(root, file)
                    target_image_path = f'../../../DataSet/IAM-Online/Resized_Dataset/Test/Images/'
