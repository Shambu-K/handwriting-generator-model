...
The network architecture is as follows. The sample image of the required handwriting is fed to the transformer encoder. The encoder consists of a ResNet-18 based CNN encoder

The loss function of this network, though complex, when broken down and related to each component of the network, is fairly intuitive. The loss function has three main components.
First, the generative adversarial loss and cycle loss is defined by the discriminator's ability to distinguish a generated image from a dataset-fed image thus ensuring that any newly generated image bears a likeness to the dataset. Therefore, the first loss component relates to the discriminator.
